 ...
 */
function vif_post_classes( $classes, $class, $post_id ) {
	$classes[] = 'entry';

	return $classes;
}
add_filter( 'post_class', 'vif_post_classes', 10, 3 );


/**
 * Add a pingback url auto-discovery header for single posts, pages, or attachments.
 */
function vif_pingback_header() {
	if ( is_singular() && pings_open() ) {
		echo '<link rel="pingback" href="', esc_url( get_bloginfo( 'pingback_url' ) ), '">';
	}
}
add_action( 'wp_head', 'vif_pingback_header' );

/**
 * Changes comment form default fields.
 */
function vif_comment_form_defaults( $defaults ) {
...
add_filter( 'comment_form_defaults', 'vif_comment_form_defaults' );

/**
 * Filters the default archive titles.
 */
function vif_get_the_archive_title() {
	if ( is_category() ) {
...
add_filter( 'get_the_archive_title', 'vif_get_the_archive_title' );

/**
 * Determines if post thumbnail can be displayed.
 */
function vif_can_show_post_thumbnail() {
	return apply_filters( 'vif_can_show_post_thumbnail', ! post_password_required() && ! is_attachment() && has_post_thumbnail() );
}

/**
 * Returns true if image filters are enabled on the theme options.
 */
function vif_image_filters_enabled() {
 ...
 */
function vif_post_thumbnail_sizes_attr( $attr ) {
...
add_filter( 'wp_get_attachment_image_attributes', 'vif_post_thumbnail_sizes_attr', 10, 1 );

/**
 * Returns the size for avatars used in the theme.
 */
function vif_get_avatar_size() {
	return 60;
}
 ...
 */
function vif_is_comment_by_post_author( $comment = null ) {
 ...
 */
function vif_get_discussion_data() {
 ...
 */
function vif_add_ellipses_to_nav( $nav_menu, $args ) {
	...
	if ( 'menu-1' === $args->theme_location ) :
		...
		$nav_menu .= vif_get_icon_svg( 'arrow_drop_down_ellipsis' );
		$nav_menu .= '</button>';
		...
		$nav_menu .= vif_get_icon_svg( 'chevron_left' );
...
add_filter( 'wp_nav_menu', 'vif_add_ellipses_to_nav', 10, 2 );
 ...
 */
function vif_nav_menu_link_attributes( $atts, $item, $args, $depth ) {
...
add_filter( 'nav_menu_link_attributes', 'vif_nav_menu_link_attributes', 10, 4 );
 ...
 * Add a dropdown icon to top-level menu items
 */
function vif_add_dropdown_icons( $output, $item, $depth, $args ) {
	...
	if ( in_array( 'mobile-parent-nav-menu-item', $item->classes, true ) && isset( $item->original_id ) ) {
		...
		$link = sprintf(
			...
			vif_get_icon_svg( 'chevron_left', 24 )
		);
		...
		$icon = vif_get_icon_svg( 'keyboard_arrow_down', 24 );
...
add_filter( 'walker_nav_menu_start_el', 'vif_add_dropdown_icons', 10, 4 );
 ...
 */
function vif_add_mobile_parent_nav_menu_items( $sorted_menu_items, $args ) {
	static $pseudo_id = 0;
	if ( ! isset( $args->theme_location ) || 'menu-1' !== $args->theme_location ) {
...
add_filter( 'wp_nav_menu_objects', 'vif_add_mobile_parent_nav_menu_items', 10, 2 );

/**
 * Convert HSL to HEX colors
 */
function vif_hsl_hex( $h, $s, $l, $to_hex = true ) {
